								...
								<ul class="dropdown-menu">
									{foreach from=$sectionData key=itemKey item=menuItem}
										<li>
											<a
												{if $menuItem.txt}
													href="/mdemo/showTxt?sectionName={$sectionName}&title={$menuItem.title}&name={$menuItem.txt}"
												{elseif $menuItem.run}
													href="/mdemo/run?class={$menuItem.run}"
												{elseif $menuItem.source}
													target="{$menuItem.name}"
													href="/mdemo/viewSource?file={$menuItem.source}"
												{elseif $menuItem.func}
													href="/mdemo/{$menuItem.func}"
												{elseif $menuItem.url}
													href="{$menuItem.url}"
												{/if}
												{if $menuItem.target}
